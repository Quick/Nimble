...
struct AsyncPollingTests { // swiftlint:disable:this type_body_length
    @Test func testToPositiveMatches() async {
    ...
    @Test func testToEventuallyPositiveMatches() async {
        var value = 0
    ...
    @Test func testToEventuallyNegativeMatches() async {
        let value = 0
    ...
    @Test func testToEventuallyWithAsyncExpressions() async {
        actor ExampleActor {
            private var count = 0
    ...
    @Test func testToEventuallySyncCase() async {
        await expect(1).toEventually(equal(1), timeout: .seconds(300))
    }

    @Test func testToEventuallyWaitingOnMainTask() async {
    ...
    @MainActor
    @Test func testToEventuallyOnMain() async {
        await expect(1).toEventually(equal(1), timeout: .seconds(300))
        await expect { try? await Task.sleep(nanoseconds: 10_000); return 1 }.toEventually(equal(1))
    }

    @MainActor
    @Test func testToEventuallyMatcherIsAlwaysExecutedOnMainActor() async {
    ...
    @Test func testToEventuallyWithSyncExpectationAlwaysExecutesExpressionOnMainActor() async {
    ...
    @Test func testToEventuallyWithAsyncExpectationDoesNotNecessarilyExecutesExpressionOnMainActor() async {
    ...
    @MainActor
    @Test func testToEventuallyWithAsyncExpectationDoesExecuteExpressionOnMainActorWhenTestRunsOnMainActor() async {
        // This prevents a "Class property 'isMainThread' is unavailable from asynchronous contexts; Work intended for the main actor should be marked with @MainActor; this is an error in Swift 6" warning.
        // However, the functionality actually works as you'd expect it to, you're just expected to tag things to use the main actor.
        func isMainThread() -> Bool { Thread.isMainThread }
    ...
    @Test func testToEventuallyWithCustomDefaultTimeout() async {
    ...
    @Test func testWaitUntilWithCustomDefaultsTimeout() async {
    ...
    @Test func testWaitUntilPositiveMatches() async {
        await waitUntil { done in
            done()
        }
        await waitUntil { done in
    ...
    @Test func testWaitUntilUsingSendable() async {
        await waitUntil { done in
    ...
    @Test func testWaitUntilTimesOutIfNotCalled() async {
        await failsWithErrorMessage("Waited more than 1.0 second") {
            await waitUntil(timeout: .seconds(1)) { _ in return }
        }
    }

    @Test func testWaitUntilTimesOutWhenExceedingItsTime() async throws {
        actor WaitState {
    ...
    @Test func testWaitUntilNegativeMatches() async {
    ...
    @Test func testWaitUntilDetectsStalledMainThreadActivity() async {
    ...
    @Test func testWaitUntilErrorsIfDoneIsCalledMultipleTimes() async {
    ...
    @Test func testWaitUntilDoesNotCompleteBeforeRunLoopIsWaiting() async {
    ...
    @Test func testSubjectUnderTestIsReleasedFromMemory() async {
    ...
    @Test func testToNeverPositiveMatches() async {
        var value = 0
        deferToMainQueue { value = 1 }
    ...
    @Test func testToNeverNegativeMatches() async {
        var value = 0
    ...
    @Test func testToAlwaysPositiveMatches() async {
        var value = 1
    ...
    @Test func testToAlwaysNegativeMatches() async {
        var value = 1
