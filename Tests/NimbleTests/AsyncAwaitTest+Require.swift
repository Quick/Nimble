...
final class AsyncAwaitRequireTest: XCTestCase { // swiftlint:disable:this type_body_length
    func testToPositiveMatches() async throws {
        ...
        _ = try await require { try await someAsyncFunction() }.to(equal(1))
    }
    ...
    func testToEventuallyPositiveMatches() async throws {
        ...
        _ = try await require { value }.toEventually(equal(1))

        deferToMainQueue { value = 0 }
        _ = try await require { value }.toEventuallyNot(equal(1))
    }

    func testToEventuallyNegativeMatches() async {
        let value = 0
        await failsWithErrorMessage("expected to eventually not equal <0>, got <0>") {
            _ = try await require { value }.toEventuallyNot(equal(0))
        }
        ...
        await failsWithErrorMessage("unexpected error thrown: <\(errorToThrow)>") {
            _ = try await require { try self.doThrowError() }.toEventually(equal(1))
        }
        await failsWithErrorMessage("unexpected error thrown: <\(errorToThrow)>") {
            _ = try await require { try self.doThrowError() }.toEventuallyNot(equal(0))
        }
    }
    ...
    func testToEventuallyWithAsyncExpressions() async throws {
        ...
        _ = try await require { await subject.value() }.toEventually(equal(2))
    }

    func testToEventuallySyncCase() async throws {
        _ = try await require(1).toEventually(equal(1), timeout: .seconds(300))
    }
    ...
    func testToEventuallyWaitingOnMainTask() async throws {
        ...
        _ = try await require(EncapsulatedValue.executed).toEventually(beTrue())
    }

    @MainActor
    func testToEventuallyOnMain() async throws {
        _ = try await require(1).toEventually(equal(1), timeout: .seconds(300))
        _ = try await require { try? await Task.sleep(nanoseconds: 10_000); return 1 }.toEventually(equal(1))
    }
    ...
    func testToEventuallyMatcherIsAlwaysExecutedOnMainActor() async throws {
        ...
        _ = try await require(MySubject()).toEventually(equal(MySubject()))
    }

    func testToEventuallyWithSyncExpectationAlwaysExecutesExpressionOnMainActor() async throws {
        _ = try await require(Thread.isMainThread).toEventually(beTrue())
        _ = try await require(Thread.isMainThread).toEventuallyNot(beFalse())
        _ = try await require(Thread.isMainThread).toAlways(beTrue(), until: .seconds(1))
        _ = try await require(Thread.isMainThread).toNever(beFalse(), until: .seconds(1))
    }
    ...
    func testToEventuallyWithAsyncExpectationDoesNotNecessarilyExecutesExpressionOnMainActor() async throws {
        ...
        _ = try await require(isMainThread()).toEventually(beFalse())
        _ = try await require(isMainThread()).toEventuallyNot(beTrue())
        _ = try await require(isMainThread()).toAlways(beFalse(), until: .seconds(1))
        _ = try await require(isMainThread()).toNever(beTrue(), until: .seconds(1))
    }
    ...
    func testToEventuallyWithAsyncExpectationDoesExecuteExpressionOnMainActorWhenTestRunsOnMainActor() async throws {
        ...
        _ = try await require(isMainThread()).toEventually(beTrue())
        _ = try await require(isMainThread()).toEventuallyNot(beFalse())
        _ = try await require(isMainThread()).toAlways(beTrue(), until: .seconds(1))
        _ = try await require(isMainThread()).toNever(beFalse(), until: .seconds(1))
    }
    ...
    func testSubjectUnderTestIsReleasedFromMemory() async throws {
        ...
        if let sub = subject {
            _ = try await require(sub.count).toEventually(equal(0), timeout: .milliseconds(100))
    ...
    func testToNeverPositiveMatches() async throws {
        var value = 0
        deferToMainQueue { value = 1 }
        _ = try await require { value }.toNever(beGreaterThan(1))

        deferToMainQueue { value = 0 }
        _ = try await require { value }.neverTo(beGreaterThan(1))
    }

    func testToNeverNegativeMatches() async {
        var value = 0
        await failsWithErrorMessage("expected to never equal <0>, got <0>") {
            _ = try await require { value }.toNever(equal(0))
        }
        await failsWithErrorMessage("expected to never equal <0>, got <0>") {
            _ = try await require { value }.neverTo(equal(0))
        }
        await failsWithErrorMessage("expected to never equal <1>, got <1>") {
            deferToMainQueue { value = 1 }
            _ = try await require { value }.toNever(equal(1))
        }
        await failsWithErrorMessage("expected to never equal <1>, got <1>") {
            deferToMainQueue { value = 1 }
            _ = try await require { value }.neverTo(equal(1))
        }
        await failsWithErrorMessage("unexpected error thrown: <\(errorToThrow)>") {
            _ = try await require { try self.doThrowError() }.toNever(equal(0))
        }
        await failsWithErrorMessage("unexpected error thrown: <\(errorToThrow)>") {
            _ = try await require { try self.doThrowError() }.neverTo(equal(0))
        }
    }
    ...
    func testToAlwaysPositiveMatches() async throws {
        ...
        _ = try await require { value }.toAlways(beGreaterThan(0))

        deferToMainQueue { value = 2 }
        _ = try await require { value }.alwaysTo(beGreaterThan(1))
    }

    func testToAlwaysNegativeMatches() async {
        var value = 1
        await failsWithErrorMessage("expected to always equal <0>, got <1>") {
            _ = try await require { value }.toAlways(equal(0))
        }
        await failsWithErrorMessage("expected to always equal <0>, got <1>") {
            _ = try await require { value }.alwaysTo(equal(0))
        }
        await failsWithErrorMessage("expected to always equal <1>, got <0>") {
            deferToMainQueue { value = 0 }
            _ = try await require { value }.toAlways(equal(1))
        }
        await failsWithErrorMessage("expected to always equal <1>, got <0>") {
            deferToMainQueue { value = 0 }
            _ = try await require { value }.alwaysTo(equal(1))
        }
        await failsWithErrorMessage("unexpected error thrown: <\(errorToThrow)>") {
            _ = try await require { try self.doThrowError() }.toAlways(equal(0))
        }
        await failsWithErrorMessage("unexpected error thrown: <\(errorToThrow)>") {
            _ = try await require { try self.doThrowError() }.alwaysTo(equal(0))
        }
    }
}
