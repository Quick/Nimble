...
struct AsyncTimerSequence: AsyncSequence {
    ...
    let interval: NimbleTimeInterval
    ...
    struct AsyncIterator: AsyncIteratorProtocol {
        ...
        func nextDeadline() -> Date {
            ...
            let next = last.advanced(by: interval.timeInterval)
            if next < now {
                let nextTimestep = interval.timeInterval * ((now.timeIntervalSince(next)) / interval.timeInterval).rounded(.up)
                return last.advanced(by: nextTimestep)
            } else {
                return next
            }
        }
        ...
        mutating func next() async -> Void? {
            ...
            let nextDeadlineNanoseconds = UInt64(Swift.max(0, next.timeIntervalSinceNow * 1_000_000_000))
            do {
                try await Task.sleep(nanoseconds: nextDeadlineNanoseconds)
            } catch {
                return nil
            }
            last = next
