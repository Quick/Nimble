...
public func map<T, U>(_ transform: @escaping @Sendable (T) throws -> U, _ matcher: Matcher<U>) -> Matcher<T> {
...
/// So, you could write `expect(myObject).to(lens(\.someIntValue, equal(3))`.
/// This is also useful in conjunction with ``satisfyAllOf`` to do a partial equality of an object.
public func map<T, U>(_ transform: @escaping @Sendable (T) async throws -> U, _ matcher: some AsyncableMatcher<U>) -> AsyncMatcher<T> {
