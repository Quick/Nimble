...
func beLessThanOrEqualTo<T: NMBComparable>(expectedValue: T?) -> MatcherFunc<T?> {
    return MatcherFunc { actualExpression, failureMessage in
        failureMessage.postfixMessage = "be less than or equal to <\(expectedValue)>"
        let actualValue = actualExpression.evaluate()
        return actualValue && actualValue!.NMB_compare(expectedValue) != NSComparisonResult.OrderedDescending
    }
}
...
extension NMBObjCMatcher {
    class func beLessThanOrEqualToMatcher(expected: NMBComparable?) -> NMBObjCMatcher {
